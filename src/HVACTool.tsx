...
interface UserInfo { name: string; email: string; phone: string; city: string; }
type UiErrors = Partial<Record<keyof UserInfo, string>>;
interface PlacementResult { score: number; verdict: string; tips: string[]; }
interface BestPlacement extends PlacementResult { name: string; }
...
//  HVAC ENGINE
// ════════════════════════════════════════════════════════
function calcBTU(l: number, w: number, _h: number, p: number, win: number, sun: string, rt: string) {
...
function acLabel(btu: number) {
  const t=btu/12000;
  if(t<=1) return "1 Ton"; if(t<=1.5) return "1.5 Ton";
  if(t<=2) return "2 Ton"; return "2+ Ton / Central";
}
function coolTime(btu: number, area: number){ return Math.round((area*25)/(btu/60)); }
function monthlyCost(btu: number, hrs=8){ return Math.round((btu/3412)*hrs*30*7); }
function evalPlacement(l: number, w: number, pl: string, door: string | null, furn: string): PlacementResult {
...
function bestPlacement(l: number, w: number): BestPlacement {
...
function cvsCalc(rooms: number, hrs: number): CvsResult {
...
function ReportTemplate({result,ai,lang,pdfRef}: ReportTemplateProps){
...
function PDFButton({result,ai,lang}: PDFButtonProps){
...
function RoomCanvas({l,w,placement,animated,showComfort}: RoomCanvasProps){
...
function MultiCanvas({rooms,type,animated}: MultiCanvasProps){
  const ref=useRef<HTMLCanvasElement>(null),fr=useRef(0),tm=useRef<ReturnType<typeof setTimeout> | null>(null);
...
interface AIQuery { l: number; w: number; rt: string; btu: number; bp: string; cost: number; }
async function getAI(data: AIQuery, lang: string): Promise<string> {
  try {
