...
import SmallComponentCard from "./components/cards/SmallComponentCard";
import StatCard from "./components/cards/StatCard";

type SunLevel = "Low" | "Medium" | "High";
type RoomType = "Bedroom" | "Living Room" | "Office" | "Commercial" | "Industrial";
interface QuickResult { btu: number; ac: string; cost: number; time: number; }

// ═══════════════════════════════════════════════════════════════════
//  QUICK HVAC ENGINE
// ═══════════════════════════════════════════════════════════════════
const qBTU  = (l: number, w: number, sun: SunLevel, rt: RoomType) => {
  const sf = ({Low:0.9,Medium:1.0,High:1.2} as Record<SunLevel, number>)[sun]||1.0;
  const rf = ({Bedroom:0.9,"Living Room":1.1,Office:1.2,Commercial:1.4,Industrial:1.6} as Record<RoomType, number>)[rt]||1.0;
  return Math.round(l*w*20*sf*rf);
};
const qAC   = (btu: number) => { const t=btu/12000; return t<=1?"1 Ton":t<=1.5?"1.5 Ton":t<=2?"2 Ton":t<=3?"3 Ton":"Central AC"; };
const qCost = (btu: number) => Math.round((btu/3412)*8*30*7);
const qTime = (btu: number, area: number) => Math.round((area*25)/(btu/60));
...
function QuickTool() {
  ...
  const [f,setF]=useState<{l:number,w:number,sun:SunLevel,rt:RoomType}>({l:1,w:1,sun:"Medium",rt:"Bedroom"});
    ...
    <div style={{maxWidth:680,margin:"0 auto"}}>
      <div style={{display:"grid",gridTemplateColumns:isMobile?"1fr 1fr":"1fr 1fr 1fr 1fr",gap:14,marginBottom:20}}>
        ...
        <div>
          ...
          <select value={f.sun} onChange={e=>set("sun",e.target.value as SunLevel)} style={selStyle}>
        ...
        <div>
          ...
          <select value={f.rt} onChange={e=>set("rt",e.target.value as RoomType)} style={selStyle}>
...
//  MAIN WEBSITE
// ═══════════════════════════════════════════════════════════════════
interface ClimewaveWebsiteProps { setView: Dispatch<SetStateAction<string>>; }
export default function ClimewaveWebsite({setView}: ClimewaveWebsiteProps) {
  const isMobile = useIsMobile();
